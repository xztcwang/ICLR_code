...
datadir = "/home/tkw5356/_gcflow/data/graph_datasets"
...
def load_citation(dataset_str="cora", normalization="AugNormAdj", porting_to_torch=True, data_path=datadir,
    ...
    if task_type == "full":
        ...
        idx_remain = range(len(y) + 500,min(idx_test))
    else:
    ...
    if porting_to_torch:
        ...
        idx_remain=torch.LongTensor(idx_remain)
        idx_test = torch.LongTensor(idx_test)
        degree = torch.LongTensor(degree)
    learning_type = "transductive"
    return adj, features, labels, idx_train, idx_val, idx_remain, idx_test, degree, learning_type
...
def data_loader(dataset, data_path=datadir, normalization="AugNormAdj", porting_to_torch=True, task_type="semi"):
    if dataset == "reddit":
        ...
        return adj, train_adj, features, train_features, labels, idx_train, idx_val, idx_remain, idx_test, degree, learning_type
