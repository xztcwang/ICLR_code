...
class COMPUTERS(Dataset, metaclass=Named):
    num_classes = 10
    class_weights = None
    ignored_index = -100
    stratify = True
    def __init__(self, path='/home/tkw5356/_gcflow/data/graph_datasets/', part='all', remake=False):
        super().__init__()
        X = torch.load(path+'cs_x.pt')
        Y = torch.load(path+'cs_y.pt')
        edge_index = torch.load(path + 'cs_edge_index.pt')
        train_mask = torch.load(path + 'cs_train_mask.pt')
        val_mask = torch.load(path + 'cs_val_mask.pt')
        test_mask = torch.load(path + 'cs_test_mask.pt')
        remain_mask = torch.load(path + 'cs_remain_mask.pt')
