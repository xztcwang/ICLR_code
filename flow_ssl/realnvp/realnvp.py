...
from flow_ssl.realnvp.coupling_layer import CouplingLayerGraph
...
class RealNVPBase(nn.Module):
    # def forward(self, x_adj_mat):
    #     return self.body(x_adj_mat)
    def forward(self, x_adj_mat):
        z = self.body(x_adj_mat)
        # z0 = z[0].reshape((z[0].shape[0], -1))
        # z0 = self.relu(z0)
        # z0 = self.linear_mapping(z0)
        return z
    ...
    #     nll = -(prior_ll + logdet)
    #     return nll
    def nll(self, x_adj_mat, idx, y=None, label_weight=1.):
        z = self(x_adj_mat)
        logdet = self.logdet()
        matdet_list = self.matdet()
        z0 = z[0].reshape((z[0].shape[0], -1))
        prior_ll = self.prior.log_prob(z0[idx], y, label_weight=label_weight)
        #nll = -(prior_ll + logdet[idx])
        return -prior_ll, -logdet, z0, matdet_list

    # def nll1(self, x_adj_mat, idx, y=None, label_weight=1.):
    #     z = self(x_adj_mat)
...
class RealNVPGraph(RealNVPBase):
    def __init__(self, num_coupling_layers=6, in_dim=1433, hidden_dim=64, n_class=7, init_zeros=False, **gcn_config):
        super(RealNVPGraph, self).__init__()
        self.body = iSequential(*[
            CouplingLayerGraph(in_dim,
                               hidden_dim,
        ...
        # self.relu = nn.ReLU()
        # self.linear_mapping = nn.Linear(in_dim, n_class)
