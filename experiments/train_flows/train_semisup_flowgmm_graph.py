...
from experiments.train_flows import cutils
...
def create_linear_transform(linear_transform_type, dim_in, dropout_ratio, device, flowlayer):
    if linear_transform_type == 'permutation':
        ...
        return transforms.CompositeTransform([
            ...
            #transforms.SVDLinear(dim_in, num_householder=10, identity_init=True)
        ], dropout_ratio, device, flowlayer)
    else:
        raise ValueError
...
class SemiFlowNSF(Trainer):
    ...
    def logStuff(self, step, minibatch=None):
        ...
        with Eval(self.model), torch.no_grad():
            ...
            metrics['test_macroF1'] = self.macro_f1
            ...
            if metrics['val_Acc']>self.best_val_acc:
                ...
                x_adj = (minibatch[1], self.adj_mat)
                pred_testlabel = self.model.prior.classify((self.model.log_prob(x_adj=x_adj))[0][self.idx_test]).\
                    type_as(self.test_labels)
                pred_testlabel_np = pred_testlabel.detach().cpu().numpy()
                true_testlabel_np = self.test_labels.detach().cpu().numpy()
                macro_f1 = f1_score(true_testlabel_np, pred_testlabel_np, average='macro')
            ...
            if minibatch:
                ...
                if step == self.totalep - 1:
                    ...
                    self.MI, self.SScore, centroids= utils.clustering_measure(z, pred, self.class_num, self.all_labels, self.idx_train, self.device)
